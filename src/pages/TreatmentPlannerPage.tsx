...
export function TreatmentPlannerPage({ onNavigate }: { onNavigate: (page: Page) => void }) {
  ...
  return (
    <div className="page planner-page">
      <SiteHeader page="planner" onNavigate={onNavigate} />
      ...
      <main className="planner-main">
        ...
        {!done ? <section className="planner-card">
          ...
          <div className="result-grid">{treatmentResults.map(([name, desc, score], index) => <article key={name} className="result-card"><div className={'result-badge badge-' + index}>{index + 1}</div><h3>{name}</h3><p>{desc}</p><strong>{score}</strong><button onClick={() => onNavigate('diseases')}>查看详情 -&gt;</button></article>)}</div>
